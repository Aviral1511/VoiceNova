    ...
    return (
        <div className='w-full h-screen bg-cover flex justify-center items-center' style={{ backgroundImage: `url(${bg})` }}>
            <form onSubmit={handleSubmit} className='w-[90%] h-160 max-w-150 bg-[#00000074] backdrop-blur shadow-lg shadow-blue-700 flex flex-col items-center justify-center gap-5 p-5'>
                ...
                <h1 className='text-white font-semibold text-3xl mb-10'>Login to  <span className='text-blue-700 font-bold'>VoiceNova</span></h1>
                ...
                <button className='h-12 min-w-30 rounded-full bg-gray-300 text-lg font-semibold text-black cursor-pointer mt-3' disabled={loading}>{loading ? "Logging in .." : "Login"}</button>
                <p className='text-[15px] font-medium text-white '>Don't have an account ? <span className='text-blue-400 font-bold cursor-pointer' onClick={() => navigate(`/signup`)}>Signup</span></p>
